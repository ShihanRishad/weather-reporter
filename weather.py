...
def create_directories(base_dir):
    if not os.path.exists(base_dir): # Check if base directory exists
        os.makedirs(base_dir) # Create base directory
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S") # Get current date-time
    report_dir = os.path.join(base_dir, current_time) # Create a new directory with current date-time
    os.makedirs(report_dir) # Create report directory
    return report_dir
...
def main():
    ...
    if latitude and longitude: # Do this if not empty
        ...
        report = (f"Current temperature in {city} is {temperature}{utemperature}\n"
